...
public:
    ...
    void PerformLabeling()
    {
        ...
        ET.Alloc(UPPER_BOUND_4_CONNECTIVITY); // Memory allocation of the labels solver
...
#define ACTION_5 img_labels_row[c] = ET.Merge(img_labels_row_prev[c], img_labels_row[c - 1]); // x <- p + s
...
#define ACTION_5 img_labels(r, c) = ET.MemMerge((unsigned)img_labels(r - 1, c - 1), (unsigned)img_labels(r, c - 1)); // x <- p + s
...
private:
    double Alloc()
    {
        ...
        double ls_t = ET.Alloc(UPPER_BOUND_4_CONNECTIVITY, perf_);
